...
export default function Allplayers (): JSX.Element {
  ...
  const playerList = searchTerm.length < 1 ? players.data : searchResults

  function searchHandler (searchTerm): void {
    setSearchTerm(searchTerm)
    if (searchTerm !== '') {
      const newPlayerList = players.data.filter(player => {
        return Object.values(player).join(' ').toLowerCase().includes(searchTerm.toLowerCase())
      })
      setSearchResults(newPlayerList)
    } else setSearchResults(players.data)
  }

  function sortAZ (): void {
    setSortedArrayZA([])
    setSortedArrayAZ(players.data.sort((a, b) => {
  ...
  function sortZA (): void {
    setSortedArrayAZ([])
    setSortedArrayZA(players.data.sort((a, b) => {
      const x = a.osuUsername.toLowerCase()
      const y = b.osuUsername.toLowerCase()
      if (y < x) { return -1 }
      if (y > x) { return 1 }
